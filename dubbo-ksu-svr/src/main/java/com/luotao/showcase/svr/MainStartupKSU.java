...
public class MainStartupKSU {
    public static final String CONTAINER_KEY = "dubbo.container";

    public static final String SHUTDOWN_HOOK_KEY = "dubbo.shutdown.hook";

    private static final Logger logger = LoggerFactory.getLogger(MainStartupKSU.class);
    ...
    public static void main(String[] args) {
        ...
        try {
                ...
                Runtime.getRuntime().addShutdownHook(new Thread() {
                    public void run() {
                        for (Container container : containers) {
                            ...
                            synchronized (MainStartupKSU.class) {
                                running = false;
                                MainStartupKSU.class.notify();
                            }
                        }
                    }
                });
            ...
            logger.info("Dubbo admin-svr service server starting...");
            for (Container container : containers) {
                container.start();
                logger.info("Dubbo " + container.getClass().getSimpleName() + " started!");
            }
            logger.info("Dubbo admin-svr service server started!");
